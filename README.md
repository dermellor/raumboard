...
Kids never log in. A school has two secrets, and the PIN does nearly all the work:

| | Teacher PIN | Admin login |
| --- | --- | --- |
| What it is for | booking on a device, and the Verwaltung: rooms, classes, kids | the Excel/CSV import of class lists |
| Entered by | a teacher, once per device and on every entry to the Verwaltung | the school's responsible person |
...
Signing in and out of the school's account happens at the foot of the start page,
next to „Verwaltung". Two tabs inside the Verwaltung ask for the account's
password before they open: „Kinder importieren", because the import brings
personal data in from outside and can replace the data of the whole school in one
go, and „Zugangsdaten", because the password and the PIN are changed there. One
confirmation covers both until the page is left. The forms behind it then ask for
the new password or the new PIN only.
