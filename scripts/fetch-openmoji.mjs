...
async function run() {
  ...
  for (const hex of hexes) {
    ...
    const pair = colorPair(svg)
    if (pair) colors[hex] = pair
  }

  // One scale factor per SVG on disk (see "optical normalization" above).
  const scales = {}
  for (const hex of hexes) {
    const file = `${DIR}/${hex}.svg`
    if (!existsSync(file)) continue
    const scale = inkScale(await readFile(file, 'utf-8'))
    if (scale !== 1) scales[hex] = scale
  }
...
// Ink-area normalization: 1 means "canvas average, leave alone", factors
// below shrink near-full-bleed artwork slightly, factors above lift thin or
// flat designs to the same optical presence. No entry means scale 1.
export const EMOJI_SCALE: Record<string, number> = {
${Object.keys(scales)
  .sort()
  .map((h) => `  '${h}': ${scales[h]},`)
  .join('\n')}
...
`,
  ...
  console.log(`manifest: ${hexes.length} emoji, ${Object.keys(colors).length} with colors, ${Object.keys(scales).length} scaled (src/emoji-assets.ts)`)
}
